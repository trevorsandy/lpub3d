...
// STRING UTILITIES
char* LDSearchDirs::copyString(const char *string, size_t pad)
{
  if (string)
    {
...
wchar_t* LDSearchDirs::copyString(const wchar_t *string, size_t pad)
{
  if (string)
    {
...
char* LDSearchDirs::cleanedUpPath(const char* path)
{
...
void LDSearchDirs::replaceStringCharacter(char* string, char oldChar, char newChar, int
                            repeat)
{
  char* spot;
...
void LDSearchDirs::stripTrailingPathSeparators(char* path)
{
  if (path)
    {
...
char* LDSearchDirs::componentsJoinedByString(char** array, int count, const char* separator)
{
  size_t length = 0;
  int i;
...
char** LDSearchDirs::componentsSeparatedByString(const char* string, const char* separator, int &count)
{
  int i;
...
char* LDSearchDirs::stringByReplacingSubstring(const char* string, const char* oldSubstring,
...
char* LDSearchDirs::directoryFromPath(const char* path)
{
  if (path)
    {
