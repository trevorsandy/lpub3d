           ...
           * Non called out sub-model are processed in findPage() */
            ...
            if (lpub->ldrawFile.isSubmodel(type) && callout && ! noStep && ! buildMod.ignore) {
                ...
                if (callout->bottom.modelName != thisType) {
                    ...
                    returnValue = static_cast<TraverseRc>(drawPage(callout, line, calloutOpts));
...
#endif
                        const TraverseRc drc = static_cast<TraverseRc>(drawPage(&lpub->page, addLine, pageOptions));
          ...
          writeToTmpFutures.append(QtConcurrent::run([ i,message, progressMessage, fileName, fileType,
              ...
              if (!displayModel) {
                  ...
                  if (Gui::doFadeStep) {
                      ...
                      emit gui->messageSig(LOG_INFO_STATUS, tr("Writing %1'%2' to temp folder...").arg(fileType).arg(fadeFileNameStr));
                  ...
                  if (Gui::doHighlightStep) {
                      ...
                      emit gui->messageSig(LOG_INFO_STATUS, tr("Writing %1'%2' to temp folder...").arg(fileType).arg(highlightFileNameStr));
