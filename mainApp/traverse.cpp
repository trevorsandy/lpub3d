          ...
          if (! opts.flags.partIgnore) {
              ...
              if (token.size() == 15) {
                  ...
                  if (contains) {
                      ...
                      bool buildModRendered = Preferences::buildModEnabled && (buildMod.ignore2 || getBuildModRendered(buildMod.key, colorType));
    ...
    bool            buildModActionChange/*false*/)
{
  ...
  if (findPage(view,scene,meta,empty/*addLine*/,findOptions) == HitBuildModAction && Preferences::buildModEnabled) {
    ...
    if (modelStack.size()) {
      ...
      // get last modelStack item (where we stopped in findPage) - step number and current where
      ...
      // set new current where, new step number and renderParentModel from next up modelStack modelName or top model
