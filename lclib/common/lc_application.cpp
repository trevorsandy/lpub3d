...
/*** LPub3D Mod - run search directories ***/
	...
	if (Preferences::archivePartsOnLaunch) {
		...
		emit Application::instance()->splashMsgSig(tr("70% - Skip parts archive per application preference..."));
	...
	emit Application::instance()->splashMsgSig(tr("75% - Archive libraries loading..."));
/*** LPub3D Mod end ***/
...
lcStartupMode lcApplication::Initialize(const QList<QPair<QString, bool>>& LibraryPaths, QMainWindow *parent)
{
	...
	***/
	...
	mPreferences.mStudCylinderColorEnabled = mOptions.StudCylinderColorEnabled;
	mPreferences.mStudCylinderColor = mOptions.StudCylinderColor;
	mPreferences.mPartEdgeColorEnabled = mOptions.PartEdgeColorEnabled;
	mPreferences.mPartEdgeColor = mOptions.PartEdgeColor;
	mPreferences.mBlackEdgeColorEnabled = mOptions.BlackEdgeColorEnabled;
	mPreferences.mBlackEdgeColor = mOptions.BlackEdgeColor;
	mPreferences.mDarkEdgeColorEnabled = mOptions.DarkEdgeColorEnabled;
	...
	lcGetPiecesLibrary()->SetStudStyle(mOptions.StudStyle, false, mOptions.StudCylinderColorEnabled);
