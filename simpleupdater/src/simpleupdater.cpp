...
/*!
 * If the \a initial update parameter is set to \c true, this
 * function will reflect that status.
 *
 * \sa checkForUpdates()
 */
...
void SimpleUpdater::onCheckingFinished (void)
{
    ...
    if (newerVersionAvailable() && m_show_update_available)
    {
        ...
        if (_message.exec() == QMessageBox::Yes){
            ...
            QSettings Settings;
            Settings.setValue("Updates/IgnoreVersion", m_versionData);
        }
    }
...
void SimpleUpdater::checkDownloadedVersion (QNetworkReply *reply)
{
    ...
    if(reply->error() == QNetworkReply::NoError)
    {
        ...
        m_versionData = reply->readAll();
        const char *update = m_versionData;
        m_latest_version = QString::fromUtf8(m_versionData);

        QSettings Settings;
        QByteArray ignoreUpdate = Settings.value("Updates/IgnoreVersion", QByteArray()).toByteArray();

        if (m_initialUpdate && ignoreUpdate == m_versionData)
        {
            _new_update = false;
        ...
        } else if (sscanf(update, "%d.%d.%d", &majorVersion, &minorVersion, &patchVersion) == 3)
        {
            ...
            showErrorMessage(tr("Error parsing update information."));
        ...
        showErrorMessage("Error connecting to the update server: " + reply->errorString());
    ...
    if (!m_changelog_url.isEmpty() && newerVersionAvailable())
    {
        ...
        m_updateRequest.setUrl(QUrl(m_changelog_url));
